...
const Sparkle = ({ className = "", style = {}, delay = 0, size = "2px" }) => (
  <motion.div
    ...
    animate={{ scale: [0.8, 1.4, 0.8], opacity: [0, 0.9, 0] }}
    transition={{ duration: 2.5, delay, repeat: Infinity, repeatDelay: 1.5, ease: "easeInOut" }}
  />
);
...
const Hero: React.FC = () => {
  ...
  const mouseY = useMotionValue(0);  // ✅ FIXED: Added back
    ...
    >
      ...
      <div className="relative z-10 container mx-auto max-w-7xl px-4">
        <motion.div className="text-center mb-16 lg:mb-24" variants={containerVariants}>
          <motion.p className="text-lg sm:text-xl md:text-2xl mb-6 text-white/90 font-medium" variants={itemVariants}>
            Hi! I'm <span className="text-purple-400 bg-purple-500/20 px-2 py-1 rounded-lg">Madan Patel</span>
          </motion.p>
          
          <motion.h1 className="text-4xl sm:text-6xl lg:text-7xl font-black mb-8 bg-gradient-to-r from-white via-purple-100 to-blue-100 bg-clip-text text-transparent leading-[0.9] tracking-tight" variants={itemVariants}>
            OGL Developer | Oracle Guided Learning Specialist
          </motion.h1>
          
          <motion.div className="text-xl sm:text-2xl lg:text-3xl text-white/80 mb-12 max-w-3xl mx-auto leading-relaxed" variants={itemVariants}>
            Dynamic Oracle Guided Learning (OGL) Developer transforming complex user experiences in Oracle applications.
          </motion.div>
          ...
          <motion.div className="flex flex-col sm:flex-row gap-4 justify-center items-center" variants={itemVariants}>
            <a href="/MadanPatel_Resume.pdf" download className="px-8 py-4 bg-gradient-to-r from-purple-600 to-blue-600 text-white font-semibold rounded-xl hover:shadow-[0_0_30px_rgba(139,92,246,0.4)] transition-all duration-300 text-lg">
              Download Resume
            </a>
            <a href="#contact" className="px-8 py-4 border-2 border-white/30 text-white font-semibold rounded-xl backdrop-blur-sm hover:bg-white/10 transition-all duration-300 text-lg">
              Get In Touch
            </a>
          </motion.div>
        </motion.div>
